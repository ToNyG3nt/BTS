...
\begin{description}
    ...
    L'\textbf{urgence} représente la vitesse à laquelle il faut intervenir selon le ressenti de l'utilisateur. L'\textbf{impact} mesure le nombre de personnes ou de services affectés. La \textbf{priorité} est calculée automatiquement par GLPI en combinant les deux : un incident urgent avec un fort impact sera traité en premier.

    \item[\textbf{Q2 -- Pourquoi le Wi-Fi en salle est plus prioritaire qu'une installation logicielle ?}]~\\
    Le Wi-Fi indisponible en salle bloque simultanément l'accès à Internet pour un groupe entier et impacte directement le déroulement du cours. L'installation d'un logiciel ne concerne qu'un seul utilisateur et n'est pas bloquante immédiatement.
    ...
    Sans solution documentée, l'équipe ne peut pas capitaliser sur l'expérience acquise. Si l'incident se reproduit, le technicien devra repartir de zéro. De plus, l'utilisateur ne comprend pas ce qui a été fait et ne peut pas prévenir une récidive.
\end{description}
\end{qabox}
...
\begin{description}
    ...
    Le suivi technique est réservé aux techniciens : il peut contenir des informations sensibles (identifiants, configurations réseau, commandes système). L'article de la base de connaissances est rédigé dans un langage accessible aux utilisateurs finaux, sans informations sensibles.
