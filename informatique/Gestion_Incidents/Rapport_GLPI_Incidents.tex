...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth, keepaspectratio]{screen/image.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth, keepaspectratio]{screen/14.png}
...
\begin{description}
    \item[Description :] L'utilisateur \texttt{enseignant1} signale qu'il ne parvient plus à se connecter au réseau Wi-Fi depuis son poste.
    \item[Suivi technique :] Vérification de l'état du point d'accès concerné depuis la console réseau. Test de connexion depuis un autre appareil pour isoler le problème (poste ou infrastructure).
    \item[Statut :] Nouveau -- ticket en attente d'attribution.
\end{description}

\subsubsection*{Ticket 3 -- Ma session windows est bloqué}
\begin{description}
    \item[Description :] L'utilisateur \texttt{enseignant1} ne peut plus ouvrir sa session Windows. Le message d'erreur indique que les identifiants sont incorrects.
    \item[Suivi technique :] Vérification que le verrou Majuscules n'est pas activé. Test du compte sur un autre poste confirmant le blocage du compte. Réinitialisation du mot de passe via le technicien.
    \item[Statut :] En cours (Attribué) -- ticket assigné à \texttt{technicien1}.
\end{description}

\subsubsection*{Ticket 4 -- Accès à une imprimante}
\begin{description}
    \item[Description :] L'utilisateur \texttt{enseignant1} demande l'accès à une imprimante réseau pour imprimer ses documents.
    \item[Suivi technique :] Identification de l'imprimante concernée. Ajout du compte utilisateur dans le groupe d'impression correspondant.
    \item[Statut :] En cours (Attribué) -- ticket assigné à \texttt{technicien1}.
\end{description}

\subsubsection*{Ticket 5 -- Installation d'un logiciel validé par le responsable}
\begin{description}
    \item[Description :] L'utilisateur \texttt{enseignant1} demande l'installation d'un logiciel sur son poste, après validation préalable du responsable.
    \item[Suivi technique :] Vérification de la validation dans la liste des logiciels autorisés. Téléchargement depuis la source officielle et installation sur le poste.
    \item[Solution :] Logiciel installé et fonctionnel. L'utilisateur a été informé.
    \item[Statut :] Résolu.
\end{description}
