...
Avant d'installer GLPI, j'ai vérifié que WAMP était correctement installé et fonctionnel sur mon poste. L'icône WAMP est devenue verte, indiquant que tous les services (Apache, MySQL, PHP) sont actifs.

\begin{figure}[H]
    \centering
    % capture_wamp_ok.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_wamp_ok.png}
    \caption{Interface WAMP -- tous les services actifs (icône verte)}
\end{figure}
...
Apache & 2.4.x \\
PHP    & 8.x.x \\
MySQL / MariaDB & 10.x.x \\
\hline
\end{tabular}
\end{center}

\textit{(À compléter avec les versions réelles affichées dans le menu WAMP.)}

\begin{figure}[H]
    \centering
    % capture_phpmyadmin.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_phpmyadmin.png}
    \caption{Page d'accueil de phpMyAdmin}
\end{figure}
...
\begin{description}
    ...
    \item[MySQL / MariaDB :] Système de gestion de base de données qui stocke toutes les informations de GLPI (utilisateurs, tickets, matériel\ldots).
...
% ============================================================
\section{Exercice 2 -- Installation de GLPI en local}
% ============================================================

\subsection{Travail réalisé}

J'ai téléchargé l'archive GLPI et extrait le dossier dans le répertoire web de WAMP :
...
Dans phpMyAdmin, j'ai créé la base de données et l'utilisateur dédié via les commandes SQL suivantes :
...
\subsection{Lancement de l'assistant d'installation}

J'ai lancé l'installation depuis le navigateur :
\begin{lstlisting}[numbers=none]
http://localhost/glpi/public
\end{lstlisting}

J'ai suivi l'assistant jusqu'à la création des tables, puis effectué la première connexion avec le compte administrateur par défaut (\texttt{glpi} / \texttt{glpi}).

\begin{figure}[H]
    \centering
    % capture_glpi_install_fin.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_glpi_install_fin.png}
    \caption{Fin de l'assistant d'installation GLPI}
\end{figure}

\begin{figure}[H]
    \centering
    % capture_glpi_dashboard.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_glpi_dashboard.png}
    \caption{Tableau de bord GLPI après la première connexion}
\end{figure}

\subsection{Réponses aux questions}

\begin{description}
    \item[Q1 -- Pourquoi créer un utilisateur MySQL dédié plutôt qu'utiliser \texttt{root} ?]
    Le compte \texttt{root} possède des droits complets sur toutes les bases de données. En cas de compromission de GLPI, un attaquant aurait accès à tout le serveur MySQL. Un utilisateur dédié avec des droits limités à la seule base \texttt{glpi} réduit considérablement la surface d'attaque.
...
\begin{figure}[H]
    \centering
    % capture_utilisateurs.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_utilisateurs.png}
...
\begin{figure}[H]
    \centering
    % capture_categories.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_categories.png}
...
\begin{figure}[H]
    \centering
    % capture_ticket_nouveau.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_ticket_nouveau.png}
    \caption{Ticket nouvellement créé (Ticket 1 -- Session Windows)}
\end{figure}

\begin{figure}[H]
    \centering
    % capture_ticket_en_cours.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_ticket_en_cours.png}
    \caption{Ticket en cours de traitement avec suivi technique (Ticket 3 -- Wi-Fi)}
\end{figure}

\begin{figure}[H]
    \centering
    % capture_ticket_resolu.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_ticket_resolu.png}
    \caption{Ticket résolu avec solution documentée (Ticket 2 -- Imprimante)}
\end{figure}
...
\begin{figure}[H]
    \centering
    % capture_base_connaissances.png
    \includegraphics[width=0.85\textwidth, height=7cm, keepaspectratio]{capture_base_connaissances.png}
    \caption{Article de la base de connaissances lié au Ticket 1}
\end{figure}
