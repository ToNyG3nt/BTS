...
\end{lstlisting}
\vspace{4pt}
\key{Héritage} : \lstinline[style=csharp]{class Etudiant : Personne} \\
\key{Interface} : contrat de méthodes (pas d'implémentation) \\
\key{Encapsulation} : \lstinline[style=csharp]{private}, \lstinline[style=csharp]{public}, \lstinline[style=csharp]{protected} \\
\key{Polymorphisme} : méthode redéfinie dans classe fille (\lstinline[style=csharp]{override})
\end{tcolorbox}
...
\midrule
Afficher & \lstinline{Console.Write} & \lstinline{print()} \\
Lire & \lstinline{Console.ReadLine} & \lstinline{input()} \\
Si/Sinon & \lstinline{if / else \{\}} & \lstinline{if / else:} \\
Boucle for & \lstinline{for(int i=0;..)} & \lstinline{for i in range:} \\
Fonction & \lstinline{public int Fn()} & \lstinline{def fn():} \\
Commentaire & \lstinline{// texte} & \lstinline{# texte} \\
Tableau & \lstinline{int[] tab} & \lstinline{liste = []} \\
\bottomrule
\end{tabularx}
\end{tcolorbox}
...
\begin{itemize}
  ...
  \item Héritage C\# : \lstinline[style=csharp]{class Etudiant : Personne}
  \item Console.WriteLine = afficher + saut de ligne
  \item \lstinline[style=python]{range(0, 10)} = de 0 à 9 inclus
  \item Un constructeur s'appelle avec \lstinline[style=csharp]{new NomClasse()}
\end{itemize}
\end{multicols}
\end{tcolorbox}
